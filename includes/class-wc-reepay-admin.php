			...
			if ( $order = wc_get_order( $post->ID ) ) {
				...
				if ( in_array( $payment_method, WC_ReepayCheckout::PAYMENT_METHODS ) ) {
					add_meta_box(
						...
						__( 'Reepay Payment', 'woocommerce-gateway-reepay-checkout' ),
						array(
							$this,
							'meta_box_payment',
						),
					...
					/* add_meta_box(
						'reepay-payment-actions',
						__( 'Reepay Subscription', 'woocommerce-gateway-reepay-checkout' ),
						array(
							$this,
				...
				if ( 1 == $value || false === $value ) {
					if ( $gateway->can_capture($order) ) {
						...
						try {
							...
							$order->update_status(
								$from,
								sprintf( __('Order status rollback. %s', 'woocommerce-gateway-reepay-checkout'), $message )
							);
						}
					}
				}
