...
class WC_Reepay_Api {
	...
	public function request( $method, $url, $params = array() ) {
		...
		switch ( $code ) {
			case 0:
				...
				return new WP_Error( $errno, sprintf( 'Error: %s. Code: %s.', $error, $errno ) );
			case 1:
				return new WP_Error( $info['http_code'], sprintf( 'Invalid HTTP Code: %s', $info['http_code'] ) );
			case 2:
			case 3:
			...
			case 5:
				if ( mb_strpos( $body, 'Request rate limit exceeded', 0, 'UTF-8' ) !== false ) {
					if ( $this->request_retry ) {
						...
						return new WP_Error( 0, 'Reepay: Request rate limit exceeded' );
					}
				...
				if ( JSON_ERROR_NONE === json_last_error() && isset( $data['code'] ) ) {
					return new WP_Error( $data['code'], sprintf( 'API Error: %s.', $data['error'] ) );
				}

				return new WP_Error( $info['http_code'],
					sprintf( 'API Error (request): %s. HTTP Code: %s', $body, $info['http_code'] ) );
			default:
				return new WP_Error( $info['http_code'], 'Unknown error.' );
		}
	}
	...
	public function capture_payment( $order, $amount ) {
		...
		return $this->settle( $order, $amount );
	}
	 ...
	 * @throws Exception
	 */
	public function settle( WC_Order $order, $amount = null ) {
		...
		if ( $amount <= 0 ) {
			return new WP_Error( 0, 'Wrong amount' );
		}
		...
		if ( is_wp_error( $result ) ) {
			...
			//$order->add_order_note(
				//sprintf( __( 'Failed to settle "%s". Error: %s.', 'woocommerce-gateway-reepay-checkout' ),
				//wc_price( $amount ),
				//$result->get_error_message()
			//) );
		...
		if ( $order_data['authorized_amount'] === $order_data['settled_amount'] ) {
			$message = sprintf(
				__( 'Payment has been settled. Transaction: %s', 'woocommerce-gateway-reepay-checkout' ),
				$result['transaction']
			);
	...
	public function cancel( WC_Order $order ) {
		...
		if ( ! $order->has_status( 'cancelled' ) ) {
			$message = __( 'Payment has been cancelled.', 'woocommerce-gateway-reepay-checkout' );
	...
	public function refund( WC_Order $order, $amount = null, $reason = null ) {
		...
		$message = sprintf( __( 'Refunded: %s. Credit Note Id #%s. Reason: %s', 'woocommerce-gateway-reepay-checkout' ),
			wc_price( $amount ),
		...
		switch ( $result['state'] ) {
			case 'pending':
				WC_Reepay_Order_Statuses::update_order_status(
					...
					'pending',
					sprintf(
						__( 'Transaction is pending. Amount: %s. Transaction: %s', 'woocommerce-gateway-reepay-checkout' ),
			...
			case 'cancelled':
				...
				if ( ! $order->has_status( 'cancelled' ) ) {
					$order->update_status( 'cancelled',
						__( 'Payment has been cancelled.', 'woocommerce-gateway-reepay-checkout' ) );
				} else {
